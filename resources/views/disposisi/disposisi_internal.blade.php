    ...
    <div class="container-xl">
        ...
        <div class="row">
            <div class="col-md-12 mb-1">
                ...
                @endif
                <div class="card">
                    <form action="{{ url('surat/disposisi/internal/'. $disID) }}" method="post">
                        @csrf
...
{{--                            DIKOMENTARI DIKARENAKAN TIDAK MEMAKAI TABLE PERNERIMA QRCODE --}}
                            ...
                            {{--<div class="form-group row mb-3" id="opd-select">
                                <label class="form-label col-md-3 col-form-label">Asal Disposisi</label>
                                <div class="col">
                                    <select name="dis_asal" id="dis_asal" class="form-control select2 @error('dis_asal') is-invalid @enderror">
                                        <option></option>
                                        @foreach($opd as $o)
                                            <option {{ old('dis_asal') == $o->id_opd ? 'selected':'' }} value="{{ $o->id_opd }}">{{ $o->nama_opd }}</option>
                                        @endforeach
                                    </select>
                                    <div class="form-hint">Silahkan pilih asal disposisi sesuai dengan terakhir tracking surat.</div>
                                    @error('dis_asal')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row mb-3" id="opd-select">
                                ...
                                <div class="col">
                                    <select name="dis_tujuan[]" id="dis_tujuan" class="form-control select2 @error('dis_tujuan') is-invalid @enderror" @if($user->level == 'superadmin' || $user->level == 'admin' || $user->opd()->first()->level_pj != env('OPD_BIASA')) multiple @endif>
                                        <option></option>
                                        @foreach($opd as $o)
