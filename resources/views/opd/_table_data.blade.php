<table class="table card-table table-vcenter text-nowrap datatable">
    ...
    @forelse($table as $key => $tb)
        <tr>
            <td><input onchange="checkValue()" value="{{ $tb->id_opd }}" class="form-check-input m-0 align-middle check-table" type="checkbox" aria-label="Pilih data" id="check-id-{{ $tb->id_opd }}"></td>
            <td><span class="text-muted">{{ $table->firstItem() + $key }}</span></td>
            <td><a href="{{ url('opd/edit/'.$tb->id_opd) }}" class="text-reset" tabindex="-1">{{ $tb->nama_opd }}</a> <br> <small>{{ $tb->alias_opd }}</small></td>
            <td>
                {{ $tb->alamat_opd ?? '-' }}
            </td>
            <td>
            ...
            <td class="text-end">
                            <span class="dropdown">
                              ...
                              <div class="dropdown-menu dropdown-menu-end">
                                <a class="dropdown-item" href="{{ url('opd/edit/'.$tb->id_opd) }}">
                                ...
                                <a class="dropdown-item" href="javascript:void(0);" onclick="hapus({{ $tb->id_opd }})">
            ...
            <td colspan="8" class="text-center">
                <div class="empty">
                    ...
                    <div class="empty-action">
                        <a href="{{ url('opd/create') }}" class="btn btn-primary">
