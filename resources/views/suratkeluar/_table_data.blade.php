            ...
            <div class="card">
                <div class="row row-0">
                    <div class="col-4 position-relative">
                        <label class="form-check form-switch m-0 top-0 start-0 position-absolute m-2">
                            <input onchange="checkValue()" value="{{ $tb->id_qr }}" id="check-id-{{ $tb->id_qr }}" class="form-check-input position-static" type="checkbox">
                        </label>
                        @if(\Illuminate\Support\Facades\Storage::exists($tb->qrcode) && $tb->qrcode != null)
                        <a href="{{ \Illuminate\Support\Facades\Storage::url($tb->qrcode) }}" class="image-link">
                            <img src="{{ \Illuminate\Support\Facades\Storage::url($tb->qrcode) }}" class="w-100 h-100 object-cover">
                        </a>
                        @else
                    ...
                    <div class="col">
                        <div class="card-body">
                            ...
                            <h4 class="card-title">No. {{ $tb->no_surat }}</h4>
                            <span>{{ \Carbon\Carbon::parse($tb->tgl_surat)->isoFormat('dddd, D/MMMM/YYYY') }}</span><br>
{{--                            <span><b>{{ $tb->opd->nama_opd }}</b></span>--}}
                            <span>{{ \Illuminate\Support\Str::words(\Illuminate\Support\Str::ucfirst($tb->perihal), 5, '...') }}</span>
                        </div>
                        <div class="card-footer">
                            <div class="btn-list justify-content-center">
                                <a href="{{ url('surat/detail/'.\Vinkla\Hashids\Facades\Hashids::encode($tb->id_qr)) }}" target="_blank" class="btn btn-primary btn-icon" aria-label="Button">
                                ...
                                @if($user['level'] != 'user')
                                <a href="{{ url('surat/keluar/edit/' . $tb->id_qr) }}" class="btn btn-warning btn-icon" aria-label="Button">
                                    <!-- Download SVG icon from http://tabler-icons.io/i/brand-facebook -->
                                ...
                                @endif
                                <a target="_blank" href="{{ \Illuminate\Support\Facades\Storage::url($tb->berkas) }}" class="btn btn-success btn-icon" aria-label="Button">
                                    <!-- Download SVG icon from http://tabler-icons.io/i/brand-facebook -->
                                ...
                                @if(!$user['level'] != 'user')
                                <a href="javascript:void(0)" class="btn btn-danger btn-icon" aria-label="Button" onclick="hapus({{ $tb->id_qr }})">
                                    <!-- Download SVG icon from http://tabler-icons.io/i/brand-facebook -->
