<table class="table card-table table-vcenter text-nowrap datatable">
    ...
    @forelse($table as $key => $tb)
        <tr>
            ...
            <td><a href="{{ url('galeri/edit/'.$tb->id) }}" class="text-reset" tabindex="-1">{{ $tb->title }}</a><br>
                <span class="badge @if($tb->active) bg-success @else bg-danger @endif">{{ $tb->active ? 'Active' : 'Non-aktif' }}</span>
            </td>

            <td>{{ \Carbon\Carbon::parse($tb->tanggal)->isoFormat('dddd, D/MMMM/YYYY') }}</td>
            <td>{{ \Illuminate\Support\Str::words($tb->content, 5) }}</td>
            <td>
                <a href="{{ \Illuminate\Support\Facades\Storage::url($tb->picture) }}" class="image-link">
                    <img src="{{ \Illuminate\Support\Facades\Storage::url($tb->picture) }}" height="50">
                </a>
            </td>
            <td class="text-end">
                            <span class="dropdown">
                              ...
                              <div class="dropdown-menu dropdown-menu-end">
                                <a class="dropdown-item" href="{{ url('galeri/edit/'.$tb->id) }}">
