<table class="table card-table table-vcenter text-nowrap datatable">
    ...
    @forelse($table as $key => $tb)
        <tr>
            ...
            <td><a href="{{ url('galeri/kategori/edit/'.$tb->id) }}" class="text-reset" tabindex="-1">{{ $tb->label }}</a><br>
                <small>Last update:@if($tb->updated_at == null) Belum pernah diupdate @else {{ \Carbon\Carbon::parse($tb->updated_at)->isoFormat('dddd, D/M/YYYY') }} @endif</small>
            </td>

            <td><span class="badge @if($tb->active) bg-success @else bg-danger @endif">{{ $tb->active ? 'Active' : 'Non-aktif' }}</span></td>
            <td width="5%" class="text-center">{{ $tb->galeri_count }} Foto</td>

            <td class="text-end">
                            <span class="dropdown">
                              <button class="btn dropdown-toggle align-text-top" data-bs-boundary="viewport" data-bs-toggle="dropdown">Actions</button>
                              <div class="dropdown-menu dropdown-menu-end">
                                <a class="dropdown-item" href="{{ url('galeri/kategori/edit/'.$tb->id) }}">
            ...
            <td colspan="8" class="text-center">
                <div class="empty">
                    ...
                    <div class="empty-action">
                        <a href="{{ url('galeri/kategori/create') }}" class="btn btn-primary">
...
<div class="d-flex align-items-center m-2">
    ...
    <p class="m-0 text-muted">Menampilkan <span>{{ $table->firstItem() }}</span> sampai <span>{{ $table->lastItem() }}</span> dari <span>{{ $table->total() }}</span> data</p>
